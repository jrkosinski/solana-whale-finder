...
def fetch_transaction_safe(sig):
    try:
        return sig, get_transaction(sig)
    except Exception:
        return sig, None
...
def build_leaderboard():
    ...
    print("Fetching pump.fun transactions in parallel...")
    tx_results = []
    with ThreadPoolExecutor(max_workers=30) as executor:
        futures = [executor.submit(fetch_transaction_safe, rec["signature"]) for rec in pump_txs]
        for future in tqdm(as_completed(futures), total=len(futures), desc="pump.fun txs"):
            sig, tx = future.result()
            if tx:
                tx_results.append(tx)

    for rec in tqdm(tx_results, desc="processing pump.fun txs"):
    ...
    for wallet in tqdm(list(ledgers.keys()), desc="scan wallets sells"):
        ...
        tx_results = []
        with ThreadPoolExecutor(max_workers=30) as executor:
            futures = [executor.submit(fetch_transaction_safe, sig) for sig in sig_list]
...
def find_pumpfun_txs_multithread(program_ids, hours_back=7):
    ...
    def fetch_signatures_for_program(prog):
        local_results = []
        before = None
        while True:
            try:
            ...
            for s in sigs:
                bt = s.get("blockTime")
                if not bt or bt < cutoff_unix:
                    # stop if we've gone beyond the cutoff
                    sigs = []
                    break
